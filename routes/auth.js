...
router.get("/orders", requireLogin, async (req, res) => {
  try {
    ...
    const orderItems = currentUserOrders
...
router.post("/login", (req, res) => {
  ...
  validateUser(username, password, (err, user) => {
    ...
    req.session.currentUser = user.userId; // Spara den aktuella användarens ID
    req.session.isOnline = true; // Ändra variabeln till true
    req.session.role = user.role; // Sätt rollen här
...
// Endpoint för att kontrollera sessionen
